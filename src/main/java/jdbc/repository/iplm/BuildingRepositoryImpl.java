...
public class BuildingRepositoryImpl implements BuildingReponsitory {
	...
	public void queryNormal(Map<String, Object> map, List<String> typeCode, StringBuilder where) {
		for (Map.Entry<String, Object> item : map.entrySet()) {
			if (!item.getKey().equals("staffId") && !item.getKey().equals("typeCode")
					&& !item.getKey().startsWith("area") && !item.getKey().startsWith("price")) {
	...
	public List<BuildingEntity> findAll(Map<String, Object> map, List<String> typeCode) {
		...
		List<BuildingEntity> results = DAL_Util.getInstance().executeQuery(sql, BuildingEntity.class).toList();
...
//		}
	...
	public boolean create(BuildingEntity buildingEntity) {
		String sql = "INSERT INTO Building (name, street, ward, districtId, floorArea, rentPrice, serviceFee, brokerageFee) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
		return DAL_Util.getInstance().executeUpdate(sql, buildingEntity, EnumDB.CREATE);
	}

	@Override
	public boolean update(BuildingEntity buildingEntity) {
		String sql = "UPDATE Building SET name = ?, street = ?, ward = ?, districtId = ?, floorArea = ?, rentPrice = ?, serviceFee = ?, brokerageFee = ? WHERE id = ?";
		return DAL_Util.getInstance().executeUpdate(sql, buildingEntity, EnumDB.UPDATE);
	}
	
	public boolean deleteOne (Long id) {
		String sql = "DELETE FROM Building WHERE id = ?";
		return DAL_Util.getInstance().executeUpdate(sql, id, EnumDB.DELETE);
	}
	
	@Override
	public void delete(List<Long> ids) {
		for (Long id : ids) {
			if(deleteOne(id)) System.out.println("delete success (id: " +id + ")");
			else System.out.println("delete failed (id: " +id + ")");
		}
	}
}
